...
#import "RNPagingPdfViewComponentView.h"
...
@interface RNPagingPdfViewComponentView () <RCTRNPagingPdfViewViewProtocol>
@end

@implementation RNPagingPdfViewComponentView {
    UIView *_pdfView;
}
...
+ (ComponentDescriptorProvider)componentDescriptorProvider {
    return concreteComponentDescriptorProvider<RNPagingPdfViewComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        static const auto defaultProps = std::make_shared<const RNPagingPdfViewProps>();
        _props = defaultProps;

        Class viewClass = NSClassFromString(@"react_native_pdf_light.PagingPdfView");
        if (viewClass) {
...
- (void)_setupEventCallbacks {
    ...
    [_pdfView setValue:^(NSDictionary *body) {
        ...
        if (strongSelf && strongSelf->_eventEmitter) {
            auto emitter = std::dynamic_pointer_cast<const RNPagingPdfViewEventEmitter>(strongSelf->_eventEmitter);
            if (emitter) {
...
- (void)updateProps:(const Props::Shared &)props oldProps:(const Props::Shared &)oldProps {
    const auto &oldViewProps = *std::static_pointer_cast<const RNPagingPdfViewProps>(_props);
    const auto &newViewProps = *std::static_pointer_cast<const RNPagingPdfViewProps>(props);
...
- (void)handleCommand:(const NSString *)commandName args:(const NSArray *)args {
    RCTRNPagingPdfViewHandleCommand(self, commandName, args);
}

#pragma mark - RCTRNPagingPdfViewViewProtocol
...
Class<RCTComponentViewProtocol> RNPagingPdfViewCls(void) {
    return RNPagingPdfViewComponentView.class;
}
